...
CPU, memory, PHP workers, disk, database connections, I/O, PIDs, backup scratch,
and network. Existing site reservations are rebuilt from `sites.json` at startup.
...
`PHP_WORKERS`, `DISK_BYTES`, `DB_CONNECTIONS`, `IO_MB`, `PIDS`,
`SCRATCH_BYTES`, and `NETWORK_MBPS`). Expensive operation pools are configured

...
type SiteProvisioner interface {
	...
	AppAdminPassword(siteID string, request site.AppAdminPasswordRequest) (site.AppAdminPasswordResult, error)
...
func (executor *Executor) runCommand(command Command) (Result, error) {
	...
	case SiteAppInstall:
		request, parseErr := parseSiteAppInstall(command.Payload)
		if parseErr != nil {
			return Result{}, parseErr
		}
		if executor.sites == nil {
		...
		installed, installErr := executor.sites.AppInstall(request.SiteID, request.toRequest())
		if installErr != nil {
	...
	case SiteAppAdminPassword:
		request, parseErr := parseSiteAppAdminPassword(command.Payload)
		if parseErr != nil {
			return Result{}, parseErr
		}
		if executor.sites == nil {
		...
		reset, resetErr := executor.sites.AppAdminPassword(request.SiteID, request.toRequest())
		if resetErr != nil {

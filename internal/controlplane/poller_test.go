...
// ErrOutboxFull stops the batch so a command is never acknowledged without a
// durable result.
func TestExecuteAndReportAbortsBatchOnOutboxFull(t *testing.T) {
	...
	if len(executed) != 1 || executed[0] != "1" {
		t.Fatalf("only the command with the full outbox should be attempted, got %v", executed)
	}
}

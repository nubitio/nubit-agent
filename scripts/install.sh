...
if [ ! -f "$env_file" ]; then
  if "$dry_run"; then
    ...
    umask 077
    {
      ...
      printf '# TLS. Caddy issues from public Let'"'"'s Encrypt by default — leave these\n'
      printf '# unset for that. Point Caddy at a private ACME CA (e.g. an internal\n'
      printf '# step-ca) by setting the directory URL, the CA root to trust, and a\n'
      printf '# bound for how long tls.letsencrypt.enable waits on an in-flight order.\n'
      printf '#NUBIT_TLS_ACME_CA=\n'
      printf '#NUBIT_TLS_ACME_CA_ROOT=\n'
      printf '#NUBIT_TLS_ACME_EMAIL=\n'
      printf '#NUBIT_TLS_ISSUE_WAIT=150s\n'

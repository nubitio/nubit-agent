...
| app | `site.app.install`, `site.app.update`, `site.app.admin-password` | Managed WordPress: runs wp-cli as the site's Unix user (`core download`/`config create` with `--force` so a partial run is retryable → `core install`), idempotent, returns the admin password once. On success it re-renders the site vhost with the hardened WordPress Caddy template (403s `xmlrpc.php`/`wp-config.php`/VCS dirs, `path_regexp` 403 for PHP anywhere under `wp-content/uploads`, 7-day non-immutable cache on static assets) and records `app: "wordpress"` on site state so domain edits and drift checks keep the template. A transient Caddy failure after install is reported as `reason: "…hardening deferred"` with `installed: true` (drift + retry converge it), never as an install failure. `site.app.update` runs `wp core/plugin/theme update` (optional `--dry-run`, per-component selection) as the site user, records per-component pass/fail and the `core` version delta, and treats a component failure as a reported result, not a command error. `site.app.admin-password` runs `wp user update <login> --user_pass=… --skip-email --skip-plugins --skip-themes` for the portal's "reset admin password" action — the password is always server-generated, returned once, and its result is not cached (a replay re-runs the reset). The per-site auto-update toggle and the backup-before-update step are control-plane concerns. Unit-tested; real-VM/container validation is the follow-up before enablement on a customer node. See ADR-005. |

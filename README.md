...
| app | `site.app.install` | Managed WordPress: runs wp-cli as the site's Unix user (download → config → install), idempotent, returns the admin password once. On success it also re-renders the site vhost with the hardened WordPress Caddy template (blocks `xmlrpc.php`/`wp-config.php`/`wp-content/uploads/*.php`/VCS dirs, long cache header on static assets) and records `app: "wordpress"` on site state so domain edits and drift checks keep the template. Unit-tested; real-VM/container validation is the follow-up before enablement on a customer node. See ADR-005. |
